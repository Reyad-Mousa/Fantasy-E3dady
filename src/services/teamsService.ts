...
export async function moveTeamMember(params: MoveMemberParams): Promise<void> {
    ...
    if (fromTeam.id === toTeam.id) throw new Error('Source and target team must be different');

    const oldMemberKey = buildMemberKey({ teamId: fromTeam.id, memberName: trimmedName });
    const newMemberKey = buildMemberKey({ teamId: toTeam.id, memberName: trimmedName });
    ...
    const [scoresSnap, memberStatsSnap, usersSnap] = await Promise.all([
        ...
        getDocs(query(collection(db, 'scores'), where('teamId', '==', fromTeam.id))),
        // member_stats referencing old team
        getDocs(query(collection(db, 'member_stats'), where('teamId', '==', fromTeam.id))),
        // User docs linked to old team by name (for name-only members)
        memberUserId
            ? getDocs(query(collection(db, 'users'), where('teamId', '==', fromTeam.id), where('name', '==', trimmedName)))
    ...
    scoresSnap.forEach(entry => {
        ...
        const docMemberUserId = typeof d.memberUserId === 'string' ? d.memberUserId.trim() : null;
        const docMemberName = typeof d.memberName === 'string' ? normalizeMemberName(d.memberName) : '';

        const matchesByKey = docMemberKey && (docMemberKey === oldMemberKey || docMemberKey === `n:${fromTeam.id}:${normalizedName}` || docMemberKey === `n:${fromTeam.id}:${normalizedName.replace(/\s+/g, '_')}`);
        ...
        if (matchesByKey || matchesByUserId || matchesByName) {
            ...
            if (newMemberKey) update.memberKey = memberUserId ? `u:${memberUserId}` : newMemberKey;
    ...
    memberStatsSnap.forEach(entry => {
        ...
        const docName = typeof d.memberName === 'string' ? normalizeMemberName(d.memberName) : '';
        const docUserId = typeof d.memberUserId === 'string' ? d.memberUserId.trim() : '';

        const matchesByKey = docKey === oldMemberKey || docKey === `n:${fromTeam.id}:${normalizedName}` || docKey === `n:${fromTeam.id}:${normalizedName.replace(/\s+/g, '_')}`;
        ...
        if (matchesByKey || matchesByUserId || matchesByName) {
            ...
            const newKey = memberUserId ? `u:${memberUserId}` : (newMemberKey || `n:${toTeam.id}:${normalizedName}`);
            statsToUpsert.push({
                key: newKey,
                data: {
                    ...d,
    ...
    const operations: Array<{ type: 'update' | 'set' | 'delete'; ref: any; data?: Record<string, unknown> }> = [];
    ...
    operations.push({
        ...
        data: { members: arrayRemove(trimmedName), memberCount: fromUpdatedMembers.length },
    });
    operations.push({
        type: 'update',
        ref: doc(db, 'teams', toTeam.id),
        data: { members: arrayUnion(trimmedName), memberCount: toTeam.members.length + 1 },
    });
    ...
    if (memberUserId) {
        ...
        (usersSnap as any).docs.forEach((userDoc: any) => {
            operations.push({ type: 'update', ref: userDoc.ref, data: { teamId: toTeam.id } });
        });
    }

    // Score updates
    scoresToUpdate.forEach(({ id, newData }) => {
        operations.push({ type: 'update', ref: doc(db, 'scores', id), data: newData });
    });
