...
import { Trophy, Users, ListTodo, BarChart3, Star, Target, Award, ArrowLeft } from 'lucide-react';
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-5 flex-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/20 border border-accent/20 text-accent-light text-[14px] font-black tracking-widest uppercase">
              ...
              Fantasy E3dady             </div>
            <h1 className="text-4xl sm:text-5xl font-black text-white leading-[1.15]">
        ...
        >
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-black text-white">تحليل نقاط المراحل</h3>
                <p className="text-[10px] text-text-muted font-bold uppercase tracking-wider">نسبة النقاط التراكمية</p>
              </div>
            </div>
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 border-t border-white/5 pt-3" dir="ltr">
                {mobileChartData.map((stage, i) => (
                  <div key={i} className="text-center">
                    <div className="text-[10px] font-black text-text-muted uppercase">{stage.shortName}</div>
                    <div className="text-sm font-black" style={{ color: stage.color }}>{stage.points}</div>
                    <div className="text-[10px] text-text-muted">{stage.count} فرق</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="mt-8 grid grid-cols-3 gap-3 border-t border-white/5 pt-6" dir="ltr">
                {stageStats.map((s, i) => (
                  <div key={i} className="text-center group cursor-default">
                    <div className="text-[10px] font-black text-text-muted mb-1 group-hover:text-white transition-colors uppercase">{s.name.split(' ')[0]}</div>
                    <div className="text-lg font-black transition-all group-hover:scale-110" style={{ color: s.color }}>{s.points}</div>
                    <div className="text-[12px] text-text-muted mt-0.5">{s.count} فرق</div>
                  </div>
                ))}
              </div>
            </>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-surface-card rounded-3xl p-6 border border-white/5 shadow-xl relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-black text-white flex items-center gap-2 text-sm uppercase tracking-tight">
                <Award className="w-4 h-4 text-accent" />
                أبطال المراحل
              </h3>
              <button onClick={() => navigate('leaderboard')} className="text-[10px] text-text-muted font-black hover:text-accent transition-colors flex items-center gap-1">
                المتصدرين
                ...
                return (
                  <div key={stage.id} className="relative bg-white/[0.02] border border-white/5 rounded-2xl overflow-hidden">
                    <div className="px-3 py-2 bg-white/[0.03] border-b border-white/5 flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-wider" style={{ color: stage.color }}>{stage.name}</span>
                    </div>

                    <div className="divide-y divide-white/5">
                      {topTeams.length > 0 ? (
                        topTeams.map((team: any, idx: number) => (
                          <div key={team.id || idx} className="flex items-center gap-3 p-3 hover:bg-white/[0.04] transition-all group">
                            <div
                            ...
                            >
                              {idx === 0 ? <Trophy className="w-4 h-4" /> : '2'}
                            </div>
                            <div className="flex-1 min-w-0">
                              <h4 className="text-sm font-bold text-text-primary truncate">{team.name}</h4>
                            </div>
        ...
        user && (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 relative z-10">
              <button onClick={() => navigate('scores')} className="btn btn-primary py-4 rounded-2xl h-full flex flex-col items-center gap-3 shadow-lg shadow-primary/20 group">
              ...
              <button onClick={() => navigate('tasks')} className="btn btn-accent py-4 rounded-2xl h-full flex flex-col items-center gap-3 shadow-lg shadow-accent/20 group text-bg font-black">
              ...
              <button onClick={() => navigate('teams')} className="btn btn-ghost py-4 rounded-2xl h-full flex flex-col items-center gap-3 border-2 border-border/30 hover:border-text-muted transition-all group">
              ...
              {user.role === 'super_admin' && (
                <button onClick={() => navigate('admin')} className="btn btn-ghost py-4 rounded-2xl h-full flex flex-col items-center gap-3 border-2 border-border/30 hover:border-text-muted transition-all group">
