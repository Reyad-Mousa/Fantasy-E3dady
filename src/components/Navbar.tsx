...
import {
  ...
  LogOut, Wifi, WifiOff, ChevronDown, UserCog, User, Activity
} from 'lucide-react';
...
export default function Navbar({ activeTab, setActiveTab, onLoginClick }: NavbarProps) {
  ...
  const mobileScrollable = filteredItems.length > 5;
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 bg-surface-card border-b border-border/50 shadow-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              {/* Connection status */}
              <div className={`hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold ${online ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'
                }`}>
              ...
              {user ? (
                <div className="relative" ref={userMenuRef}>
                  <button
                    ...
                    className="flex items-center gap-2 px-2.5 sm:px-3 py-2 rounded-xl bg-surface/50 hover:bg-surface transition-colors"
                ...
                <button type="button" onClick={onLoginClick} className="btn btn-primary text-sm py-2">
                  دخول
                </button>
              )}
        ...
        <div
          className="lg:hidden fixed bottom-0 left-0 right-0 z-50 px-4 pt-2"
          style={{ paddingBottom: 'max(1.5rem, env(safe-area-inset-bottom))' }}
        >
          <div className={`max-w-md mx-auto bg-surface-card/80 backdrop-blur-xl border border-white/10 rounded-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.4)] p-2 hide-scrollbar ${mobileScrollable ? 'overflow-x-auto' : 'overflow-x-hidden'}`}>
            <div className={`flex items-center gap-1 ${mobileScrollable ? 'min-w-max' : 'w-full'}`}>
              {filteredItems.map(item => {
                const isActive = activeTab === item.id;
                return (
                  <button
                    ...
                    key={item.id}
                    onClick={() => handleTabClick(item.id)}
                    className={`relative flex flex-col items-center gap-1 px-2.5 py-2 transition-all duration-300 ${mobileScrollable ? 'shrink-0 min-w-[68px]' : 'flex-1 min-w-0'} ${isActive ? 'text-primary' : 'text-text-muted hover:text-text-secondary'
                      }`}
                    aria-label={item.label}
                  >
                    {isActive && (
                    ...
                    <span className="relative z-10 max-w-full truncate whitespace-nowrap text-[10px] font-bold tracking-tight">
                      {item.label}
                    </span>
                    {isActive && (
                      <motion.div
